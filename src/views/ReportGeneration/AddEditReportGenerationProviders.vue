<template>
  <k-dialog @close="cancel" :max-width="400" :title="action">
    ...
    <form class="k-form" @submit.prevent="save">
        <input type="hidden" v-model="reportGenerationProviderModel.id">
        <input type="hidden" v-model="reportGenerationProviderModel.poolID">
        ...
        <div class="form-group row" :style="{ marginBottom: '1rem' }" >
          ...
          <div class="col-sm-9" v-if="!isEdit">
            <k-input
              type="text"
              :name="'name'"
              v-model="reportGenerationProviderModel.name"
              class="form-control"
              required
            ></k-input>
          </div>
           <div class="col-sm-9 text-left" v-else>
            <label class="display-data">{{reportGenerationProviderModel.name}}</label>
          </div>
        </div>
         <div class="form-group row" :style="{ marginBottom: '1rem' }">
          <label class="col-sm-3 text-left col-form-label">
          ...
          <div class="col-sm-9" v-if="!isEdit"> 
            <k-input
              type="text"
              :name="'type'"
              v-model="reportGenerationProviderModel.dbType"
              class="form-control"
              required
            ></k-input>
          </div>
           <div class="col-sm-9" v-else>
            <label class="display-data">{{reportGenerationProviderModel.dbType}}</label>
          </div>
        </div>
         <div class="form-group row" :style="{ marginBottom: '1rem' }">
          <label class="col-sm-3 text-left col-form-label">
          ...
          <div class="col-sm-9">
            <k-input
              ...
              v-model="reportGenerationProviderModel.connectionString"
              placeholder="Eg. ConnectionString=<connectionString>;Database:TestDatabase;"
              class="form-control"
              required
            ></k-input>
          </div>
         ...
         <div class="form-group row" :style="{ marginBottom: '1rem' }" v-if="!isEdit">
          ...
          <div class="col-sm-9">
            <dropdownlist class="form-control"
                          ...
                          :name="'poolName'"  
                          :option-label="'Select'"   
                          v-model="reportGenerationProviderModel.poolName"
...
export default {
  ...
  created: function () {
    if (this.$props.dataItem) {
      this.reportGenerationProviderModel = this.$props.dataItem;
    }
    ...
    if (this.reportGenerationProviderModel.id != undefined) this.isEdit = true;
  },
  methods: {
    cancel() {
      this.$emit("cancel");
    ...
    save(e) {
      console.log("SAVED");
         this.errors = [];
         if(!this.isEdit){
      if(this.reportGenerationProviderModel.poolName == "" || this.reportGenerationProviderModel.poolName == "Select" || this.reportGenerationProviderModel.poolName == undefined){
        this.errors.push("Please select a Pool Name.");
      }
      if(this.reportGenerationProviderModel.poolName && this.reportGenerationProviderModel.connectionString && this.reportGenerationProviderModel.dbType && this.reportGenerationProviderModel.name){
        this.errors = [];
        this.$emit("save");
      }
      }else{
