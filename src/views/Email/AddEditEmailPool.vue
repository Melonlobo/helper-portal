<template>
  <k-dialog @close="cancel" :max-width="400" :title="action">
    <form class="k-form" @submit.prevent="save">
        <input type="hidden" v-model="emailPoolModel.id">
...
export default {
  ...
  created: function () {
    if (this.$props.dataItem) {
      this.emailPoolModel = this.$props.dataItem;
    }
    this.action = this.$props.dialogTitle;
    if (this.emailPoolModel.id != undefined) this.isEdit = true;
  },
  methods: {
    cancel() {
      this.$emit("cancel");
