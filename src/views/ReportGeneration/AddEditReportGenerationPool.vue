<template>
  <k-dialog @close="cancel" :max-width="400" :title="action">
    <form class="k-form" @submit.prevent="save">
        <input type="hidden" v-model="reportGenerationPoolModel.id">
        ...
        <div class="form-group row" :style="{ marginBottom: '1rem' }">
          ...
          <div class="col-sm-9">
            <k-input
              type="text"
              :name="'name'"
              v-model="reportGenerationPoolModel.name"
              class="form-control"
              required
            ></k-input>
          </div>
...
export default {
  ...
  created: function () {
    if (this.$props.dataItem) {
      this.reportGenerationPoolModel = this.$props.dataItem;
    }
    this.action = this.$props.dialogTitle;
    if (this.reportGenerationPoolModel.id != undefined) this.isEdit = true;
  },
  methods: {
    cancel() {
      this.$emit("cancel");
