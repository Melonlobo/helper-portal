    ...
    <div class="smsHistories">
        <div class="container-fluid">
          <div class="panel-heading">
            <h2 class="text-left">SMS Histories</h2>
            </div>
            ...
            <div class="table-responsive">
              <table class="table mb-0 text-left sm-font">
                ...
                <tr>
                  ...
                  <td><input type="text" class="input-margin" v-model="tag" ></td>
                  <td>
                    <button
            type="submit"
            value="Search"
...
export default ({
    ...
    computed: {
        result: {
            get: function() {
                return this.smsHistories.slice(this.skip, this.take + this.skip);
            }
        }
    },
    methods:{
       pageChangeHandler: function(event) {
        ...
        async getSMSHistories() {
      console.log( this.base_url + "getSMSHistories/")
      if(!this.tag){
        this.tag = "";
      }
      try {
        axios.get(
          this.base_url_management + "getSMSHistories/"+this.selectedChannelKey.id+"/"+ this.tag).then((response) => {  
              if(response.data.result != null){
              this.smsHistories=response.data.result;
              }
              else{
                  this.smsHistories = []
              }
              this.gridDataTotal =this.smsHistories.length;
        });
      } catch (err) {
    ...
    errorPopupOk(){
     ...
     exportExcel () {
            saveExcel({
                data: this.smsHistories,
                fileName: "SMSHistories",
