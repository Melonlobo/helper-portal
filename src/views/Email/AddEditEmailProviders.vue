<template>
  <k-dialog @close="cancel" :max-width="400" :title="action">
    ...
    <form class="k-form" @submit.prevent="save">
        <input type="hidden" v-model="emailProviderModel.id">
        <input type="hidden" v-model="emailProviderModel.emailPoolID">
        ...
        <div class="form-group row" :style="{ marginBottom: '1rem' }" >
          ...
          <div class="col-sm-9" v-if="!isEdit">
            <k-input
              type="text"
              :name="'name'"
              v-model="emailProviderModel.name"
              class="form-control"
              required
            ></k-input>
          </div>
          <div class="col-sm-9 text-left" v-else>
            <label class="display-data">{{emailProviderModel.name}}</label>
          </div>
        </div>
         <div class="form-group row" :style="{ marginBottom: '1rem' }" >
          <label class="col-sm-3 text-left col-form-label">
          ...
          <div class="col-sm-9" v-if="!isEdit">
            <k-input
              type="text"
              :name="'type'"
              v-model="emailProviderModel.type"
              class="form-control"
              required
            ></k-input>
          </div>
          <div class="col-sm-9" v-else>
            <label class="display-data">{{emailProviderModel.type}}</label>
          </div>
        </div>
         <div class="form-group row" :style="{ marginBottom: '1rem' }">
          <label class="col-sm-3 text-left col-form-label">
          ...
          <div class="col-sm-9">
            <k-input
              type="text"
              :name="'configuration'"
              v-model="emailProviderModel.configuration"
              placeholder="Eg. smtp=smtp.host.com;sender=example@mail.com;port=1234;"
              class="form-control"
              required
            ></k-input>
          </div>
         ...
         <div class="form-group row" :style="{ marginBottom: '1rem' }" v-if="!isEdit">
          ...
          <div class="col-sm-9">
            <dropdownlist class="form-control"
                          ...
                          :name="'emailPoolName'"  
                          :option-label="'Select'"   
                          v-model="emailProviderModel.emailPoolName"
  ...
  methods: {
    ...
    save(e) {
      console.log("SAVED");
         this.errors = [];
         if(!this.isEdit){
      if(this.emailProviderModel.emailPoolName.name == "" || this.emailProviderModel.emailPoolName.name == "Select" || this.emailProviderModel.emailPoolName.name == undefined){
        this.errors.push("Please select a Pool Name.");
      }
      if(this.emailProviderModel.emailPoolName && this.emailProviderModel.configuration && this.emailProviderModel.type && this.emailProviderModel.name){
        this.errors = [];
        this.$emit("save");
      }
      }else{
