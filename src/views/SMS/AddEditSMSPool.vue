<template>
  <k-dialog @close="cancel" :max-width="400" :title="action">
    <form class="k-form" @submit.prevent="save">
        <input type="hidden" v-model="smsPoolModel.id">
...
export default {
  ...
  created: function () {
    if (this.$props.dataItem) {
      this.smsPoolModel = this.$props.dataItem;
    }
    this.action = this.$props.dialogTitle;
    if (this.smsPoolModel.id != undefined) this.isEdit = true;
  },
  methods: {
    cancel() {
      this.$emit("cancel");
