...
import DialogContainer from "./AddEditApplications.vue";
...
export default ({
    ...
    dialogTitle() {
      return `${this.applicationModel.id === undefined ? "Add" : "Edit"} Application`;
        }
    },
    methods: {
    ...
    async registerApplication(dataItem){
        ...
        try{
          await axios.post(this.base_url_management + "registerapplication/"+dataItem.key+"/"+dataItem.name).then((response) => {
            this.getApplications();
    ...
    async updateApplication(dataItem){
        ...
        try{
        await axios.post(this.base_url_management + "updateapplications/"+dataItem.key+"/"+dataItem.name).then((response) => {
